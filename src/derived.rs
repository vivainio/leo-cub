...
pub enum SentinelError {
    ...
    #[error("line {line}: relative depth underflows below the root")]
    DepthUnderflow { line: usize },
}
...
pub(crate) fn sentinel_content<'a>(line: &'a str, start: &str, end: &str) -> &'a str {
    line.strip_prefix(start)
        .and_then(|s| s.strip_suffix(end))
        .unwrap_or(line)
}

pub(crate) fn leading_width(line: &str) -> usize {
...
pub(crate) fn append_body(outline: &mut Outline, id: Option<&NodeId>, text: &str) {
    if let Some(node) = id.and_then(|id| outline.nodes.get_mut(id)) {
        node.body.push_str(text);
    }
}

pub(crate) fn restore_expansion(
...
pub(crate) fn referenced_nodes(positions: &[Position]) -> HashSet<NodeId> {
